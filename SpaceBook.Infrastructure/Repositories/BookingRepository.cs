...
public class BookingRepository : IBookingRepository
{
    ...
    public async Task<BookingDashboardDto> GetDashboardAsync()
    {
        ...
        {
            PendingRequests = await _context.Bookings.CountAsync(x => x.Status == "Pending"),
            Confirmed = await _context.Bookings.CountAsync(x => x.Status == "Approved"),
            Cancelled = await _context.Bookings.CountAsync(x => x.Status == "Cancelled")
        };
    }

    // Get All Bookings
    public async Task<IEnumerable<BookingDto>> GetAllAsync(BookingFilterDto filter)
    {
    ...
    // Get Booking By Id
    public async Task<BookingDetailsDto?> GetByIdAsync(int bookingId)
    {
    ...
    {
        return await _context.Bookings.AnyAsync(x => x.BookingId == bookingId);
    }
}